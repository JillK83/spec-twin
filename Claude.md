...
**Tier 1 banner wiring — `sizeEscalation` field:** `advisoryBannerText`'s severity-tier system (Phase 2, `VerdictOpenPage.tsx`) has Tier 1 (size delta escalation) unimplemented — `AuditOutput` has no field distinguishing `smart_estimate` triggered by `size_up_2`/`size_down_2` from `smart_estimate` triggered by a fabric/contract Hard Stop. Currently, a `smart_estimate` from size escalation alone produces no banner (purple card, "See brand size guide," "VERIFY BEFORE SAVING" CTA, empty banner slot). Needed: add `sizeEscalation: boolean` to `AuditOutput` (true when `sizeAdjustment > 1.5 || sizeAdjustment <= -1.5`), set in `resolver.ts`, propagated through `audit.ts`. Once present, `VerdictOpenPage.tsx` pushes the Tier 1 candidate (banner text: "A size difference this large is worth verifying — check the brand's size guide before buying.") per `VERDICT_CARD.md`'s severity-tier table. Can be bundled with `FABRIC_RIGID_TO_HIGH_STRETCH` reconciliation in the same dedicated engine session — both are small `AuditOutput`/resolver field-level changes, neither touches fiber physics or engine-v2 scope.
